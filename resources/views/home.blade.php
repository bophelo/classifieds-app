...
<section class="uk-section uk-padding-remove-bottom">
        <div class="uk-container uk-container-expand">
            ...
            <header class="uk-text-center">
                <h1 class="uk-heading-primary"><a href="{{ route('user.area.store', $country) }}">{{ $country->name }} </a></h1>
                ...
                <div style="margin-bottom: 20px;">
                    <div class="uk-card uk-card-default uk-card-hover uk-flex uk-flex-column" data-uk-scrollspy-class="uk-animation-slide-left-small">
                        <div class="uk-card-header uk-text-center">
                            <h4 class="uk-text-bold"><a href="{{ route('user.area.store', $province) }}">{{ $province->name }}</a></h4>
                            <hr class="uk-divider-icon">
                        </div>
                        @foreach ($province->children as $city)
                            <p class="uk-text-small uk-text-center uk-text-muted" style="margin-top: 0;"><a href="{{ route('user.area.store', $city) }}">{{ $city->name }}</a></p>
                        @endforeach
                    </div>
                </div>
                @endforeach
